...
class RNNLM(nn.Module):  # RNNLM类继承nn.Module类
    def __init__(self, vocab_size, embed_size, hidden_size, num_layers):
        ...
        self.w_xi = torch.randn(embed_size, hidden_size).to(device)
        self.w_hi = torch.randn(hidden_size, hidden_size).to(device)
        self.b_i = torch.zeros(hidden_size).to(device)

        self.w_xf = torch.randn(embed_size, hidden_size).to(device)
        self.w_hf = torch.randn(hidden_size, hidden_size).to(device)
        self.b_f = torch.zeros(hidden_size).to(device)

        self.w_xo = torch.randn(embed_size, hidden_size).to(device)
        self.w_ho = torch.randn(hidden_size, hidden_size).to(device)
        self.b_o = torch.zeros(hidden_size).to(device)

        self.w_xc = torch.randn(embed_size, hidden_size).to(device)
        self.w_hc = torch.randn(hidden_size, hidden_size).to(device)
        self.b_c = torch.zeros(hidden_size).to(device)
        # 输出层的全联接操作
    ...
    def forward(self, x, state):
        ...
        for t in range(x.shape[1]):
            # print(h, " ", c)
            xt = x[:, t, :].to(device)
            i = torch.sigmoid((xt @ self.w_xi) + (h_t @ self.w_hi) + self.b_i)
            f = torch.sigmoid((xt @ self.w_xf) + (h_t @ self.w_hf) + self.b_f)
            o = torch.sigmoid((xt @ self.w_xo) + (h_t @ self.w_ho) + self.b_o)
            c_tilda = torch.tanh((xt @ self.w_xc) + (h_t @ self.w_hc) + self.b_c)
            c_t = f * c_t + i * c_tilda
            h_t = o * torch.tanh(c_t)
            # y = (h @ self.w_hq) + self.b_q
            outputs.append(h_t.unsqueeze(0))
        hidden_seq = torch.cat(outputs, dim=0).to(device)
        ...
        out = hidden_seq.reshape(hidden_seq.size(0) * hidden_seq.size(1), hidden_seq.size(2))
        # 全连接
...
# 训练模型
with open('save/trainMyLSTM.txt', 'w') as f:
    for epoch in range(num_epochs):
        # 初始化为0
        states = (torch.zeros(batch_size, hidden_size).to(device),
...
# 测试语言模型
with torch.no_grad():
    with open('save/MyLSTM.txt', 'w') as f:
        # 初始化为0
        ...
        for i in range(num_samples):
            ...
            if (i + 1) % 100 == 0:
                print('生成了 [{}/{}] 个词，存储到 {}'.format(i + 1, num_samples, 'save/MyLSTM.txt'))
